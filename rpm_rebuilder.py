...
class Builder(object):
    ...
    def __init__(self, ncores=None):
        ...
        if ncores is None:
            self.log.debug('Using %s cores' % self.ncores)
            self.pool = Pool(self.ncores)
        else:
            self.log.debug('Using %s cores' % ncores)
    ...
    def main(self, filename, mock_config):
        ...
        for pkg in pkg_names:
            if pkg:
                for arch in arches:
                    mock_config = '%s-%s' %(mock_config, arch)
                    builds.append(self.pool.apply_async(build_rpm, (pkg, mock_config) ))
                    self.log.debug(pkg, mock_config)
